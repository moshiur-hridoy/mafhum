...
import { ArrowLeft, ArrowRight, BookOpen, Check, ChevronDown, ChevronRight, Flame, Heart, Lock, Sparkles, Trophy, Zap } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="app-frame"><header className="topbar"><div className="topbar-inner"><div className="logo-lockup"><div className="logo-mark">م</div><div><strong>মাফহুম</strong><span>Quran, made clear.</span></div></div><div className="header-stats"><span className="stat-streak"><Flame size={17} fill="currentColor" /> {progress.streak}</span><span className="stat-xp"><Sparkles size={17} fill="currentColor" /> {progress.xp} XP</span><span className="stat-hearts"><Heart size={17} fill="currentColor" /> {progress.hearts}</span><div className="profile-wrap"><button className={`profile-button ${profileOpen ? 'open' : ''}`} type="button" aria-label="Open profile menu" aria-expanded={profileOpen} onClick={() => setProfileOpen((value) => !value)}><img className="profile-image" src="/assets/profile-icon.svg" alt="" /><ChevronDown size={14} /></button>{profileOpen && <div className="profile-menu" role="menu"><button type="button" onClick={() => setProfileOpen(false)}>Settings</button><button type="button" onClick={() => setProfileOpen(false)}>About</button><button type="button" onClick={() => setProfileOpen(false)}>Help</button><div className="profile-divider" /><button type="button" className="logout-item" onClick={() => setProfileOpen(false)}>Log out</button></div>}</div></div></div></header><main className="page-wrap">{view === 'home' && <div className="dashboard-grid"><div className="left-column"><StreakCard streak={progress.streak} /><LessonOverview lesson={selectedLesson} /></div><LessonPath lessons={lessons} selectedId={selectedLesson.id} completedIds={progress.completedLessons} onSelect={(lesson) => { setSelectedLessonId(lesson.id); }} onStart={startLesson} /></div>}{view === 'practice' && activeLesson && <Practice lesson={activeLesson} wordIndex={wordIndex} selected={selectedAnswer} onAnswer={chooseAnswer} onContinue={continuePractice} onExit={backToHome} />}{view === 'complete' && <Complete correct={correctAnswers} total={activeLesson?.words.length || 5} streak={progress.streak} onBack={backToHome} />}</main>{progress.hearts === 0 && <button className="hearts-empty" onClick={() => dispatch(restoreHearts())}><Heart size={16} /> হার্ট আবার পূরণ করুন</button>}</div>;
}
