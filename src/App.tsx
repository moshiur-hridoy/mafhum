...
function LessonOverview({ lesson, status, onStart }: { lesson: Lesson; status: Lesson['status']; onStart: () => void }) {
  return <section className="lesson-overview">
    ...
    <div className="lesson-meta"><span><BookOpen size={15} /> {lesson.words.length} শব্দ</span><span><Sparkles size={15} /> {lesson.words.length * 3} exercises</span></div>
    <button className="primary-button lesson-start" onClick={onStart}><span>{status === 'completed' ? 'আবার practice করুন' : 'Lesson শুরু করুন'}</span><ArrowRight size={17} /></button>
  </section>;
}

function LessonPath({ lessons, selectedId, completedIds, onSelect }: { lessons: Lesson[]; selectedId: string; completedIds: string[]; onSelect: (lesson: Lesson, status: Lesson['status']) => void }) {
  const completed = new Set(completedIds);
  return <section className="path-panel"><div className="path-heading"><div><span className="overline">Basic course</span><h2>কুরআনের ১২৫টি শব্দ</h2></div><span className="path-count">{completedIds.length} / 25</span></div><div className="path-list">{lessons.map((lesson, index) => { const status: Lesson['status'] = completed.has(lesson.id) ? 'completed' : index === 0 || completed.has(lessons[index - 1].id) ? 'current' : 'locked'; return <button key={lesson.id} className={`path-item ${status} ${selectedId === lesson.id ? 'selected' : ''}`} disabled={status === 'locked'} onClick={() => onSelect({ ...lesson, status }, status)}><span className="path-node">{status === 'completed' ? <Check size={16} /> : status === 'locked' ? <Lock size={14} /> : lesson.number}</span><span className="path-copy"><small>LESSON {String(lesson.number).padStart(2, '0')} · {lesson.words.length} শব্দ</small><strong>{lesson.title}</strong><em>{lesson.subtitle}</em></span><ChevronRight size={17} className="path-arrow" /></button>; })}</div></section>;
}
...
export default function App() {
  ...
  const selectedStatus: Lesson['status'] = completed.has(selectedLesson.id) ? 'completed' : lessons.findIndex((lesson) => lesson.id === selectedLesson.id) === 0 || completed.has(lessons[lessons.findIndex((lesson) => lesson.id === selectedLesson.id) - 1]?.id) ? 'current' : 'locked';

  const startLesson = (lesson = selectedLesson) => { if (selectedStatus === 'locked' && lesson.id === selectedLesson.id) return; setActiveLesson(lesson); setWordIndex(0); setSelectedAnswer(null); setCorrectAnswers(0); setView('practice'); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  return <div className="app-frame"><header className="topbar"><div className="topbar-inner"><div className="logo-lockup"><div className="logo-mark">م</div><div><strong>মাফহুম</strong><span>Quran, made clear.</span></div></div><div className="top-course"><span>Basic</span><small>Foundation course</small></div><div className="header-stats"><span className="stat-streak"><Flame size={17} fill="currentColor" /> {progress.streak}</span><span className="stat-xp"><Sparkles size={17} fill="currentColor" /> {progress.xp} XP</span><span className="stat-hearts"><Heart size={17} fill="currentColor" /> {progress.hearts}</span></div></div></header><main className="page-wrap">{view === 'home' && <><div className="course-switch"><button className="active">Basic · ১২৫ শব্দ</button><button disabled>Advanced <small>Coming soon</small></button></div><div className="dashboard-grid"><div className="left-column"><StreakCard streak={progress.streak} /><LessonOverview lesson={selectedLesson} status={selectedStatus} onStart={() => startLesson()} /></div><LessonPath lessons={lessons} selectedId={selectedLesson.id} completedIds={progress.completedLessons} onSelect={(lesson) => { setSelectedLessonId(lesson.id); }} /></div><div className="dashboard-foot"><span>প্রতিদিন ৫টি নতুন শব্দ · ২৫টি lesson · মোট ১২৫টি শব্দ</span><span><Sparkles size={14} /> আজকের উত্তর: {Math.min(5, progress.dailyAnswers)} / 5</span></div></>}{view === 'practice' && activeLesson && <Practice lesson={activeLesson} wordIndex={wordIndex} selected={selectedAnswer} onAnswer={chooseAnswer} onContinue={continuePractice} onExit={backToHome} />}{view === 'complete' && <Complete correct={correctAnswers} total={activeLesson?.words.length || 5} streak={progress.streak} onBack={backToHome} />}</main>{progress.hearts === 0 && <button className="hearts-empty" onClick={() => dispatch(restoreHearts())}><Heart size={16} /> হার্ট আবার পূরণ করুন</button>}</div>;
}
